...
## Building nuggets
A nugget is a single file that contains a package and all of its dependencies (and can optionally include other packages and files). They can be built and saved to disk with the `amber nugget save` command or run directly without saving using the `amber nugget run` command. The primary benefit a nugget provides is portability: users can run or share nuggets by copying a single file.
